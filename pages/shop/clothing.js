...
const Category = ({ allProducts }) => {
    ...
    const handlePriceFilter = async(e) => {
        ...
        const { data } = await axios(`http://localhost:3005/products/category/clothing?min=${price.min}&max=${price.max}`);
        // console.log(data)
...
export async function getServerSideProps(context) {
    ...
    const { data } = await axios(`http://localhost:3005/products/category/clothing${queryString}`);
    if (!data.error){
        return {
