...
import { CategoryProducts } from '@/services';
...
const Category = ({ allProducts, user }) => {
    ...
    const handlePriceFilter = async(e) => {
        ...
        const data = await CategoryProducts("clothing", price.min, price.max);
        // console.log(data)
                        ...
                        <div className="col-lg-3 col-xl-3 col-xxl-3 d-none d-lg-block">
                            <div>
                                <div className="shop-cartigory">
                                    ...
                                    <p><Link href="/shop/clothing" className="active" >Clothing</Link></p>
...
export async function getServerSideProps(context) {
    ...
    try {
        // const { data } = await axios(`${URL}/products/category/clothing${queryString}`);
        // const user = await axios(`${process.env.CLIENT_ROOT}/api/user`, { headers: { cookie: cookie || '' } });
        const data = await CategoryProducts("clothing", min, max)
        if (!data){
            return {
