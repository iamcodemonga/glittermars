...
const Category = ({ allProducts, user }) => {
    ...
    const handlePriceFilter = async(e) => {
        ...
        const { data } = await axios(`${URL}/products/category/accessories?min=${price.min}&max=${price.max}`);
...
export async function getServerSideProps(context) {
    ...
    try {
        const { data } = await axios(`${URL}/products/category/accessories${queryString}`);
